...
#ifdef HSA_RUNTIME
hsa_status_t
air_packet_req_translation(hsa_agent_dispatch_packet_t *pkt, uint64_t va) {
...
int mlir_aie_init_device(aie_libxaie_ctx_t *ctx, uint32_t device_id) {
...
#ifdef HSA_RUNTIME
  printf("Initializing device with HSA runtime\n");
  if(ctx == NULL) {
  ...
  hsa_status_t iterate_agents_ret = hsa_iterate_agents(&get_aie_agents, reinterpret_cast<void*>(&(ctx->agents)));
  ...
  hsa_amd_agent_iterate_memory_pools(ctx->agents.front(),
                                     get_global_mem_pool,
  ...
  hsa_agent_get_info(ctx->agents[0], HSA_AGENT_INFO_QUEUE_MAX_SIZE, &aie_max_queue_size);

  auto queue_create_status =
      hsa_queue_create(ctx->agents[0], aie_max_queue_size, HSA_QUEUE_TYPE_SINGLE,
  ...
  air_queue_dispatch_and_wait(&(ctx->agents[0]), q, packet_id, wr_idx, &shim_pkt, true);
