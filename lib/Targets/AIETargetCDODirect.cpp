...
// The reason for  this goofy nonsense is XAie_Config and XAie_DevInst
// are anon typedefed structs that _and_ we do not want to force people to
// include xaie headers to use our utility.
struct XAie_Config_ : XAie_Config {
  XAie_Config_(uint8_t AieGen, uint64_t BaseAddr, uint8_t ColShift,
               uint8_t RowShift, uint8_t NumRows, uint8_t NumCols,
               uint8_t ShimRowNum, uint8_t MemTileRowStart,
               uint8_t MemTileNumRows, uint8_t AieTileRowStart,
               uint8_t AieTileNumRows, XAie_PartitionProp PartProp)
      : XAie_Config{AieGen,          BaseAddr,        ColShift,
                    RowShift,        NumRows,         NumCols,
                    ShimRowNum,      MemTileRowStart, MemTileNumRows,
                    AieTileRowStart, AieTileNumRows,  PartProp} {}
};

struct XAie_DevInst_ : XAie_DevInst {
  XAie_DevInst_(uint64_t BaseAddr) : XAie_DevInst{BaseAddr} {}
};

XAieCDOControl::XAieCDOControl(uint8_t hwGen, uint64_t xaieBaseAddr,
                               uint8_t xaieColShift, uint8_t xaieRowShift,
                               uint8_t xaieNumCols, uint8_t xaieNumRows,
                               uint8_t xaieShimRow, uint8_t xaieMemTileRowStart,
                               uint8_t xaieMemTileNumRows,
                               uint8_t xaieAieTileRowStart,
                               uint8_t xaieAieTileNumRows,
                               uint64_t xaiePartitionBaseAddr,
                               uint64_t npiAddr) {
  configPtr = std::make_unique<XAie_Config_>(
      hwGen, xaieBaseAddr, xaieColShift, xaieRowShift, xaieNumCols, xaieNumRows,
      xaieShimRow, xaieMemTileRowStart, xaieMemTileNumRows, xaieAieTileRowStart,
      xaieAieTileNumRows, XAie_PartitionProp{0});
  // Note this used to be
  // XAie_InstDeclare(_devInst, &configPtr);
  // but after the typedef above it becomes a bare allocation.
  // There is a warning near XAie_InstDeclare that might be important later:
  ...
  devInst = std::make_unique<XAie_DevInst_>(XAie_DevInst_{0});
...
void XAieCDOControl::addErrorHandlingToCDO() {
  TRY_XAIE_API(XAie_ErrorHandlingInit, &*devInst);
}

void XAieCDOControl::addAieElfToCDO(uint8_t col, uint8_t row,
                                    const std::string &elfPath) {
  // loadSym: Load symbols from .map file. This argument is not used when
  // __AIESIM__ is not defined.
  bool loadSym = false;
  TRY_XAIE_API(XAie_LoadElf, &*devInst, XAie_TileLoc(col, row), elfPath.c_str(),
               loadSym);
}

void XAieCDOControl::addAieElfsToCDO(DeviceOp &targetOp,
...
void XAieCDOControl::addInitConfigToCDO(DeviceOp &targetOp) {
  for (auto tileOp : targetOp.getOps<TileOp>()) {
    int col = tileOp.colIndex();
    int row = tileOp.rowIndex();
...
void XAieCDOControl::addCoreEnableToCDO(DeviceOp &targetOp) {
...
void AIE::initializeCDOGenerator(byte_ordering endianness) {
#ifndef NDEBUG
...
// This use to be just a function that took a lambda but after converting to
// the typedef inheritance above, because of std::unique_ptr not being copy
// constructible, this becomes a macro.
#define GENERATE_CDO_BINARY(OUTPUT_PATH, ...)                                  \
  do {                                                                         \
    startCDOFileStream((OUTPUT_PATH).c_str());                                 \
    FileHeader();                                                              \
    __VA_ARGS__;                                                               \
    configureHeader();                                                         \
...
void AIE::generateCDOBinariesSeparately(XAieCDOControl &ctl,
                                        const std::string &workDirPath,
                                        DeviceOp &targetOp) {
  GENERATE_CDO_BINARY(workDirPath + ps + "aie_cdo_error_handling.bin",
                      ctl.addErrorHandlingToCDO());
  GENERATE_CDO_BINARY(workDirPath + ps + "aie_cdo_elfs.bin",
                      ctl.addAieElfsToCDO(targetOp, workDirPath));
  GENERATE_CDO_BINARY(workDirPath + ps + "aie_cdo_init.bin",
                      ctl.addInitConfigToCDO(targetOp));
  GENERATE_CDO_BINARY(workDirPath + ps + "aie_cdo_enable.bin",
                      ctl.addCoreEnableToCDO(targetOp));
}

void AIE::generateCDOUnified(XAieCDOControl &ctl,
                             const std::string &workDirPath,
                             DeviceOp &targetOp) {
  GENERATE_CDO_BINARY(
      workDirPath + ps + "aie_cdo.bin", ctl.addErrorHandlingToCDO(),
      ctl.addAieElfsToCDO(targetOp, workDirPath),
      ctl.addInitConfigToCDO(targetOp), ctl.addCoreEnableToCDO(targetOp));
}
